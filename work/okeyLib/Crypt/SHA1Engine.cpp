...
namespace okey
{
	...
	void SHA1Engine::updateImpl(const void* data, unsigned length)
	{
		const BYTE* buffer = (const BYTE*) data;
		BYTE* db = (BYTE*) &_context.data[0];

		/* Update bitcount */
		if ((_context.countLo + ((uint32) length << 3)) < _context.countLo)
			_context.countHi++; /* Carry from low to high bitCount */
		_context.countLo += ((uint32) length << 3);
		_context.countHi += ((uint32 ) length >> 29);
	...
	void SHA1Engine::byteReverse(uint32* buffer, int byteCount)
	{
#ifdef _ENDIAN_LITTLE_
